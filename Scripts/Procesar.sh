...
if [ -e imagenes_tp ]; then
   ...
   echo "ERROR: La carpeta de imaganes NO EXISTE. 1ro debe ejecutar Generar.sh"
fi
...
do                       # de archivo, luego basename deja SOLO el nombre del archivo
   ...
   if [[ "$NOMBRE" =~ ^[A-Z][a-z]+ ]]; then #que empieze com mayuscula y siga con minusculas
...
echo "Todas las imaganes fueron procesadas y se guardaron en imaganes_tp_procesadas"
